...
<?php include "js/init_vars.js"; ?> //include the scripts to initialize the variables
                function initNodes(nodes) {
                    ...
                    for (var node in levels[0]) {
                        ...
                        levels[0][node].position.x = 100 * Math.sin(i * radSep);
                        levels[0][node].position.y = 100 * Math.cos(i * radSep);
                        
                        if (levels[0][node].altsplice && $("#splicing_toggle").is(":checked")) {
                            drawCyl(levels[0][node], ringColor);
                        }
                        levels[0][node].angle = i * radSep;
                    ...
                    var level_max = [];     
                    var nodes_added;  
                    var total_max_for_pathway;  
                    var render_limit = [];         
                    for (var i = 0; i < levels.length; i++) {
                        for (var parent in levels[i]) {
                            for (var child in nodes[parent].linked) { // for all of the parent's children
                                nodes_added++;        // count nodes for each level                  
                            }
                            if (level_max[nodes[parent].linked]>nodes_added)
                                level_max[nodes[parent].linked]=nodes_added;
                            nodes_added=0;
                        }
                    }                    
                    for (var i in levels[0]) {
                       total_max_for_pathway+=level_max[i]; 
                    }
                    render_limit[0]=0;
                    for (var i in levels[0]) {
                       render_limit[i]=level_max[i]/total_max_for_pathway* (2 * Math.PI);
                        // if max nodes inany level for 3 pathways are 3,5,4 limits for patway plotting are (0 to 3/12)*(2*pi), (3/12 to 8/12)*(2*pi) and (8/12 to 12/12)*(2*pi)
                        // ensurses that "widest" part of each path (in terms of total child nodes) is used to divide plane for plotting

                    }                   
                    
                    /**
                     * Rendering all children of level[0]
                     */
                    var lastParent = "";
                    var angle;
                    for (var i = 1; i < levels.length; i++) { // start at level 2 (i=1)
                        var lenA = (i + 1) * 100;
                        var numLinks = 0;
                        for (var childLinks in levels[i - 1]) {
                            numLinks += levels[i - 1][childLinks].linked.length;
                        }
                        var times = 0;
                        for (var parent in levels[i - 1]) {
                            times = 1;
                            var updated = false;
                            for (var childName in levels[i - 1][parent].linked) {
                                var child = levels[i - 1][parent].linked[childName]
                                var dir = 1;
                                if (updated)
                                    dir = -1;
                                if (levels[i - 1][parent].linked.length === 1) {
                                    angle = levels[i - 1][parent].angle;
                                }
                                else {
                                    minDist = $("#repl_slider").slider("value");
                                    d = Math.abs(minDist * times);
                                    angle = levels[i - 1][parent].angle + 1.5 * (dir * Math.asin(d / 100));
                                }
                                if (!updated)
                                    times++
                                updated = !updated;
                                nodeColor = choseColor(levels[i][child], 'mutated');
                                ringColor = choseColor(levels[i][child], 'altsplice');
                                var nY = lenA * Math.cos(angle);
                                var nX = lenA * Math.sin(angle);
                                levels[i][child].position = {};
                                levels[i][child].position.x = nX;
                                levels[i][child].position.y = nY;
                                levels[i][child].angle = angle
                                if (levels[i][child].endpoint)
                                    nodeColor = "blue";
                                else if (levels[i][child].ion)
                                    nodeColor = 'green'
                                var nodeGeom = new THREE.SphereGeometry(10, 10, 10);
                                var nodeMat = new THREE.MeshLambertMaterial({color: nodeColor});
                                addNode(levels[i][child], nodeMat, nodeGeom);
                                if (levels[i][child].altsplice && $("#splicing_toggle").is(":checked")) {
                                    drawCyl(levels[i][child], ringColor);
                                }
                            }
                        }
                    }
                            ...
                            if (levels[i][parent].family === undefined) { //don't draw CNVs or expression lines if the node is a family node (JAK made up of JAK1,JAK2...)
                                ...
                                if ($("#medrpkm_toggle").is(":checked")) { //only draw the RPKM lines is the RPKM toggle is on
                                    if (levels[i][parent].endpoint === undefined && levels[i][parent].ion === undefined) { //only draw if the node is not an endpoint
                                        ...
                                        geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + displayCNV * 20 + (levels[i][parent].expression * sign), levels[i][parent].position.y)); //top
                                        ...
                                        mesh.position.set(levels[i][parent].position.x - 10, displayCNV * 20 + (levels[i][parent].expression + 20) * sign, levels[i][parent].position.y);
                ...
                function onClick(event) {
                    ...
                    if (intersects1.length > 0 || intersects2.length > 0) { //if the intersection if with either a node or a family object
                        //find the previous node and reset its color
                    ...
                    else // there are no intersections
                    {
                        ...
                        if (INTERSECTED) {
                            ...
                            if (INTERSECTED.masterGene !== undefined) {
                                ...
                                for (path in pathLines) {
                                        if(pathLines[path].parentNode === INTERSECTED.name){
