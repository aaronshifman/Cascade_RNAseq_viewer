<html lang="en">
    ...
    <body class=" meny-left " style="-webkit-perspective: 800px; -webkit-perspective-origin-x: 0px; -webkit-perspective-origin-y: 50%;">
        ...
        <div class="header">
            ...
            <span class="header-left display-span" style="background-color: #FFFFFF">Low</span>
            <span class="header-left display-span" style="background-color: #F778A1">Intermediate</span>
            <span class="header-left display-span" style="background-color: #FF0000">High</span><br/>
                                ...
                                if (EXPR_STATE % 3 > 0) { //only draw the RPKM lines is the RPKM toggle is on
                                    if (levels[i][parent].endpoint === undefined && levels[i][parent].ion === undefined) { //only draw if the node is not an endpoint
                                        ...
                                        if (CNV_STATE % 3 < 2) {
                                            ...
                                            geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + displayCNV * 20 + (exprLevel * sign), levels[i][parent].position.y)); //top
                                        } else {
                                            if (levels[i][parent].copyNumber !== 0) {
                                                geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + sign * 20, levels[i][parent].position.y)); //bast
                                                geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + sign * 20 + (exprLevel * sign), levels[i][parent].position.y)); //top
                                            } else {
                                                geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + displayCNV * 20, levels[i][parent].position.y)); //bast
                                                geometry.vertices.push(new THREE.Vector3(levels[i][parent].position.x, 10 * sign + displayCNV * 20 + (exprLevel * sign), levels[i][parent].position.y)); //top
                                            }
                                        ...
                                        if (CNV_STATE % 3 < 2) {
                                            mesh.position.set(levels[i][parent].position.x - 10, displayCNV * 20 + (exprLevel + 20) * sign, levels[i][parent].position.y);
                                        } else {
                                            if (levels[i][parent].copyNumber !== 0) {
                                                mesh.position.set(levels[i][parent].position.x - 10, sign * 20 + (exprLevel + 20) * sign, levels[i][parent].position.y);
                                            } else {
                                                mesh.position.set(levels[i][parent].position.x - 10, displayCNV * 20 + (exprLevel + 20) * sign, levels[i][parent].position.y);
                                            }
