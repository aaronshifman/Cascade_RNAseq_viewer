...
<div class="meny" style="position: fixed; display: block; z-index: 1; -webkit-transform-origin: 100% 50%; -webkit-transition: all 0.5s ease; transition: all 0.5s ease; -webkit-transform: translateX(-100%) translateX(6px) scale(1.01) rotateY(-30deg);">
    <input type="checkbox" id="hold_toggle"><label for='hold_toggle'>Hold Menu</label><br/>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("#hold_toggle").change(function () {
            if ($(this).is(":checked")) {
                meny.configure({threshold: 0, mouse: false})
                meny.open()
                widthOffset = 260/2
                $("canvas").eq(1).css('margin-left', '130px')
            }
            else {
                meny.configure({threshold: 40, mouse: true})
                meny.open()
                widthOffset = 0
                $("canvas").eq(1).css('margin-left', '0px')
            }
        })
